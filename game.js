...
const reportButton = document.getElementById('report-button'); // ★追加
const shareGameButton = document.getElementById('share-game-button'); // ★追加
...
// (グローバル変数、ブロック定義、オブジェクト作成などは変更なし)
...
// --- イベントリスナー ---
// PCのキーボード操作
document.addEventListener('keydown', (e) => { if (!currentBlock || gameOver) return; if (e.key === 'ArrowLeft') { Body.setPosition(currentBlock, { x: currentBlock.position.x - 20, y: currentBlock.position.y }); } else if (e.key === 'ArrowRight') { Body.setPosition(currentBlock, { x: currentBlock.position.x + 20, y: currentBlock.position.y }); } else if (e.key === 'ArrowDown' || e.key === ' ') { dropBlock(); }});
// スマホのタッチ操作
gameCanvas.addEventListener('touchstart', (e) => { e.preventDefault(); if (!currentBlock || gameOver) return; isDragging = true; touchStartX = e.touches[0].clientX; blockStartX = currentBlock.position.x; touchStartTime = Date.now(); }, { passive: false });
gameCanvas.addEventListener('touchmove', (e) => { e.preventDefault(); if (!isDragging || !currentBlock) return; const touchX = e.touches[0].clientX; const deltaX = touchX - touchStartX; Body.setPosition(currentBlock, { x: blockStartX + deltaX, y: currentBlock.position.y }); }, { passive: false });
gameCanvas.addEventListener('touchend', (e) => { if (!isDragging || !currentBlock) return; const touchDuration = Date.now() - touchStartTime; const deltaX = Math.abs(e.changedTouches[0].clientX - touchStartX); if (touchDuration < 200 && deltaX < 10) { dropBlock(); } isDragging = false; }, { passive: false });

// リスタートボタン
restartButton.addEventListener('click', () => location.reload());

// ★★★ シェアボタンのロジックを追加 ★★★
const GAME_URL = "https://www.rikkiblog.net/entry/gragratower";
const GAME_TITLE = "GraGra Tower";
const HASH_TAGS = "GraGraTower,ブラウザゲーム";

// 「スコアを報告」ボタン
reportButton.addEventListener('click', (e) => {
    e.preventDefault();
    const text = `物理演算パズル『${GAME_TITLE}』で【スコア ${score}】を達成！みんなも挑戦しよう！`;
...
// 「友達に教える」ボタン
shareGameButton.addEventListener('click', (e) => {
    e.preventDefault();
    const text = `この積み上げパズル、ハマる！物理演算『${GAME_TITLE}』でハイスコアを競おう！`;
    const shareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(GAME_URL)}&hashtags=${encodeURIComponent(HASH_TAGS)}`;
    window.open(shareUrl, '_blank');
});


// (Matter.jsのイベントリスナーは変更なし)
